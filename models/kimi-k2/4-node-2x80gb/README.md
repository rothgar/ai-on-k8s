# Kimi-K2 — 4 Nodes × 2× 80 GB GPUs (TP=2, PP=4)

Serves [moonshotai/Kimi-K2-Instruct](https://huggingface.co/moonshotai/Kimi-K2-Instruct) via vLLM with FP8 quantization across **4 Kubernetes nodes**, each with 2× 80 GB GPUs. Uses tensor parallelism within each node (TP=2) and pipeline parallelism across nodes (PP=4). Total: 640 GB VRAM.
...
See also: [`../2-node-4x80gb/`](../2-node-4x80gb/)

## Requirements

| | |
|---|---|
| **Nodes** | 4 Kubernetes nodes, each with 2× GPUs ≥ 80 GB VRAM |
| **Total VRAM** | 640 GB (4 × 2 × 80 GB) |
...
```bash
kubectl apply -k https://github.com/siderolabs/ai-on-k8s//models/kimi-k2/4-node-2x80gb
```

Or from a local clone:

```bash
kubectl apply -k models/kimi-k2/4-node-2x80gb/
```
...
spec:
  ...
  sourceRef:
    kind: GitRepository
    name: ai-on-k8s
  path: ./models/kimi-k2/4-node-2x80gb
  prune: true
  timeout: 60m
  patches:
