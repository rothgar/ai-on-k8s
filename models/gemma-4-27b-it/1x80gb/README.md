# Gemma 4 27B IT — 1× 80 GB GPU

Serves [google/gemma-4-27b-it](https://huggingface.co/google/gemma-4-27b-it) via vLLM in FP8 on a single 80 GB GPU (H100, A100 80GB).

> **Note:** Gemma models require accepting Google's model license on HuggingFace before your token grants access.

See also: [`../2x40gb/`](../2x40gb/) to run across 2× 40 GB GPUs.

## Requirements

| | |
|---|---|
| **GPU** | 1× GPU with ≥ 80 GB VRAM (e.g. H100 80GB, A100 80GB) |
...
```bash
kubectl apply -k https://github.com/siderolabs/ai-on-k8s//models/gemma-4-27b-it/1x80gb
```

Or from a local clone:

```bash
kubectl apply -k models/gemma-4-27b-it/1x80gb/
```
...
spec:
  ...
  sourceRef:
    kind: GitRepository
    name: ai-on-k8s
  path: ./models/gemma-4-27b-it/1x80gb
  prune: true
  timeout: 20m
```
