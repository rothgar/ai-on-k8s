# DeepSeek-R1 — 2 Nodes × 8× 80 GB GPUs (TP=8, PP=2)

Serves [deepseek-ai/DeepSeek-R1](https://huggingface.co/deepseek-ai/DeepSeek-R1) via vLLM with FP8 quantization across **2 Kubernetes nodes**, each with 8× 80 GB GPUs (full DGX H100 nodes). Uses tensor parallelism within each node (TP=8) and pipeline parallelism across nodes (PP=2). Total: 1280 GB VRAM.
...
```bash
kubectl apply -k https://github.com/siderolabs/ai-on-k8s//models/deepseek-r1/2-node-8x80gb
```

Or from a local clone:

```bash
kubectl apply -k models/deepseek-r1/2-node-8x80gb/
```
...
spec:
  ...
  sourceRef:
    kind: GitRepository
    name: ai-on-k8s
  path: ./models/deepseek-r1/2-node-8x80gb
  prune: true
  timeout: 60m
  patches:
