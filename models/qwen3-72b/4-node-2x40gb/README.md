# Qwen3-72B — 4 Nodes × 2× 40 GB GPUs (TP=2, PP=4)

Serves [Qwen/Qwen3-72B](https://huggingface.co/Qwen/Qwen3-72B) via vLLM across **4 Kubernetes nodes**, each with 2× 40 GB GPUs. Uses tensor parallelism within each node (TP=2) and pipeline parallelism across nodes (PP=4). Total: 320 GB VRAM across 8 GPUs.

Uses [LeaderWorkerSet](https://github.com/kubernetes-sigs/lws) — the LWS controller must be installed before deploying.

See also: [`../2x80gb/`](../2x80gb/) · [`../2-node-4x40gb/`](../2-node-4x40gb/)

## Requirements

| | |
|---|---|
| **Nodes** | 4 Kubernetes nodes, each with 2× GPUs ≥ 40 GB VRAM |
| **Total VRAM** | 320 GB (4 × 2 × 40 GB) |
...
```bash
kubectl apply -k https://github.com/siderolabs/ai-on-k8s//models/qwen3-72b/4-node-2x40gb
```

Or from a local clone:

```bash
kubectl apply -k models/qwen3-72b/4-node-2x40gb/
```
...
spec:
  ...
  sourceRef:
    kind: GitRepository
    name: ai-on-k8s
  path: ./models/qwen3-72b/4-node-2x40gb
  prune: true
  timeout: 30m
```
